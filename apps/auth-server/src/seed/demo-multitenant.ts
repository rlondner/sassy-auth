...
const USERS: readonly UserSeed[] = [
  ...
  {
    email: "acme-alice@app01.io",
    firstName: "Acme",
    lastName: "Alice",
    org: "Acme",
  },
  {
    email: "acme-bob@app01.io",
    firstName: "Acme",
    lastName: "Bob",
    org: "Acme",
  },
  {
    email: "globex-admin@app01.io",
    firstName: "Globex",
    lastName: "Admin",
    org: "Globex",
    systemPerm: "org.users.manage",
  ...
  {
    email: "globex-gina@app01.io",
    firstName: "Globex",
    lastName: "Gina",
    org: "Globex",
  },
  {
    email: "globex-greg@app01.io",
    firstName: "Globex",
    lastName: "Greg",
    org: "Globex",
  },
...
async function ensureUser(
  seed: UserSeed,
  orgIdByName: Record<string, number>,
  sysPermByName: Record<string, number>,
) {
...
export async function seedDemoMultitenant() {
  ...
  const orgRows = await Promise.all(
    ORGS.map((name) => ensureOrg(app.id, name)),
  );
  ...
  const orgUsersPerm = await prisma.saPermission.findUnique({
    where: { name: "org.users.manage" },
  });
  if (!orgUsersPerm)
    throw new Error("seedDemoMultitenant requires org.users.manage to exist");
  const sysPermByName: Record<string, number> = {
    "org.users.manage": orgUsersPerm.id,
  };
