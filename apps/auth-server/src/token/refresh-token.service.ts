...
export class RefreshTokenService {
  ...
  async rotate(presentedToken: string, appPublicId: string): Promise<RotatedRefreshToken> {
    ...
    // Atomic single-use consumption + replacement creation: wrapped in an
    // interactive transaction so the two writes succeed or fail together.
    // The update is conditioned on `revokedAt: null` (a compound where,
    // hence updateMany rather than update since tokenHash alone is the
    // @id) so that if a concurrent request already rotated this same
    // still-valid token between our read above and this write, at most
    // one of the two racing callers can flip revokedAt here. Mirrors
    // OauthService.exchangeCode's atomic delete-based single-use
    // consumption of SaOauthCode.
    //
    // If `create` below throws for any reason (e.g. a DB error), throwing
    // inside an interactive transaction callback causes Prisma to roll
    // back the whole transaction automatically — so the `updateMany`'s
    // revocation of the presented token is undone too, rather than
    // leaving it revoked with no replacement ever created.
    await prisma.$transaction(async (tx) => {
      const { count } = await tx.saRefreshToken.updateMany({
        where: { tokenHash, revokedAt: null },
        data: { revokedAt: now, replacedByTokenHash: newTokenHash },
      });
      if (count !== 1) {
        // Lost the race: someone else already rotated (or revoked) this
        // token between our read and this write. Fail cleanly — the
        // other caller's rotation already completed normally, so
        // there's no family-burn to do here.
        throw new UnauthorizedException(TokenErrorCode.INVALID_GRANT);
      }

      return tx.saRefreshToken.create({
        data: {
