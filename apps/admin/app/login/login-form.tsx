  ...
  const [state, formAction, isPending] = useActionState(
    async (
      ...
      if ("twoFactor" in result && result.twoFactor) {
        router.push(
          `/login/two-factor${next ? `?next=${encodeURIComponent(next)}` : ""}`,
        );
      }
      return result;
    },
      ...
      <div className="w-full max-w-sm rounded-lg border border-[var(--border)] bg-[var(--card)] p-8 shadow-sm">
        ...
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-label-md font-semibold" htmlFor="email">
              {t("email")}
            </label>
            <input
              id="email"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-label-md font-semibold" htmlFor="password">
              {t("password")}
            </label>
            <div className="relative flex items-center">
              ...
              <button
                ...
                className="absolute right-3 flex h-6 w-6 items-center justify-center text-[var(--muted-foreground)] hover:text-[var(--foreground)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] focus-visible:ring-offset-2 rounded"
              >
                {showPassword ? (
          ...
          {"error" in state && state.error && (
            <p
              ...
              className="text-label-md text-[var(--destructive)]"
            >
          ...
          <Link
            href={
              next
                ? `/login/code?next=${encodeURIComponent(next)}`
                : "/login/code"
            }
            className="text-label-md text-[var(--primary)] hover:underline self-end"
          >
