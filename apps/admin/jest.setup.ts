...
jest.mock('@sassy-auth/ui', () => {
  ...
  const Trigger = ({ children, asChild: _asChild, ...rest }: { children?: React.ReactNode; asChild?: boolean }) =>
    React.isValidElement(children) ? React.cloneElement(children, rest) : React.createElement(React.Fragment, null, children)
  const Item = ({ children, onClick, className }: { children?: React.ReactNode; onClick?: (e: React.MouseEvent) => void; className?: string }) =>
    React.createElement('div', { role: 'menuitem', tabIndex: -1, className, onClick }, children)
  ...
  return {
    ...
    SidebarTrigger: () => React.createElement('button', { type: 'button', 'aria-label': 'Toggle Sidebar' }),
  }
})
