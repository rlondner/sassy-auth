...
describe("AppsTable", () => {
  ...
  it("clicking Delete on an ordinary app opens ConfirmDialog with the app name", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("menuitem", { name: en.apps.actions.delete }),
    );
  ...
  it("debounced search triggers listAppsAction with q", async () => {
    jest.useFakeTimers();
    (actions.listAppsAction as jest.Mock).mockResolvedValue({
      items: [],
      total: 0,
      page: 1,
      pageSize: 25,
    ...
    render(withIntl(<AppsTable initial={initial} />));
    fireEvent.change(screen.getByPlaceholderText(en.apps.search), {
      target: { value: "port" },
    });
    jest.advanceTimersByTime(400);
    await waitFor(() =>
      expect(actions.listAppsAction).toHaveBeenCalledWith({
        q: "port",
        page: 1,
        pageSize: 25,
      }),
