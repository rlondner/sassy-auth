...
jest.mock("@sassy-auth/ui", () => {
  ...
  const Item = ({
    ...
    <div role="menuitem" tabIndex={-1} className={className} onClick={onClick}>
      {children}
    </div>
  );
  return {
  ...
  items: [
    {
      publicId: "sq_p1",
      name: "apps.read",
      app: { publicId: "sq_a1", name: "Customer Portal" },
      roleCount: 0,
      userCount: 0,
      isSystem: false,
    },
    {
      ...
      app: { publicId: "sq_a2", name: "SassyAuth" },
      roleCount: 1,
      userCount: 0,
      isSystem: true,
    },
...
describe("PermissionsTable", () => {
  ...
  it("renders rows with names and the Platform badge for platform.*", () => {
    ...
    expect(
      screen.getByText(en.permissions.badges.platform),
    ).toBeInTheDocument();
  });
  ...
  it("Edit and Delete menu items are hidden for platform.* rows", () => {
    ...
    expect(
      screen.getAllByRole("menuitem", { name: en.permissions.actions.view }),
    ).toHaveLength(2);
    expect(
      screen.getAllByRole("menuitem", { name: en.permissions.actions.edit }),
    ).toHaveLength(1);
    expect(
      screen.getAllByRole("menuitem", { name: en.permissions.actions.delete }),
    ).toHaveLength(1);
  });
  ...
  it("clicking Delete opens AlertDialog with the permission name", async () => {
    ...
    const deleteItems = screen.getAllByRole("menuitem", {
      name: en.permissions.actions.delete,
    });
  ...
  it("renders the System badge for isSystem rows (and not the Platform badge)", () => {
      ...
      items: [
        {
          ...
          isSystem: true,
          app: { publicId: "sq_a1", name: "SassyAuth" },
          roleCount: 0,
          userCount: 0,
        },
      ] satisfies PermissionRow[],
    ...
    expect(
      screen.queryByText(en.permissions.badges.platform),
  ...
  it("hides Edit and Delete menu items for isSystem rows", () => {
    ...
    expect(
      screen.getByRole("menuitem", { name: en.permissions.actions.view }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("menuitem", { name: en.permissions.actions.edit }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("menuitem", { name: en.permissions.actions.delete }),
    ).not.toBeInTheDocument();
  });

  it("app filter triggers listPermissionsAction with appId", async () => {
    jest.useFakeTimers();
    (actions.listPermissionsAction as jest.Mock).mockResolvedValue({
      items: [],
      total: 0,
      page: 1,
      pageSize: 25,
    ...
    render(withIntl(<PermissionsTable initial={initial} apps={apps} />));
    fireEvent.change(screen.getByLabelText(en.permissions.filter.appLabel), {
      target: { value: "sq_a1" },
    });
    jest.advanceTimersByTime(400);
    await waitFor(() =>
      expect(actions.listPermissionsAction).toHaveBeenCalledWith({
        appId: "sq_a1",
        page: 1,
        pageSize: 25,
      }),
