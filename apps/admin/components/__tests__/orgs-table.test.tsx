...
describe("OrgsTable", () => {
  ...
  it("clicking Delete on an ordinary org opens ConfirmDialog with the org name", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("menuitem", { name: en.orgs.actions.delete }),
    );
  ...
  it("app filter triggers listOrgsAction with appId", async () => {
    jest.useFakeTimers();
    (actions.listOrgsAction as jest.Mock).mockResolvedValue({
      items: [],
      total: 0,
      page: 1,
      pageSize: 25,
    ...
    render(withIntl(<OrgsTable initial={initial} apps={apps} />));
    fireEvent.change(screen.getByLabelText(en.orgs.filter.appLabel), {
      target: { value: "sq_1" },
    });
    jest.advanceTimersByTime(400);
    await waitFor(() =>
      expect(actions.listOrgsAction).toHaveBeenCalledWith({
        appId: "sq_1",
        page: 1,
        pageSize: 25,
      }),
