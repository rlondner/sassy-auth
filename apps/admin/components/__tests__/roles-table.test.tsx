...
describe("RolesTable", () => {
  ...
  it("shows View/Edit/Delete menu items per row", () => {
    render(withIntl(<RolesTable initial={initial} apps={apps} />));
    expect(
      screen.getAllByRole("menuitem", { name: en.roles.actions.view }),
    ).toHaveLength(2);
    expect(
      screen.getAllByRole("menuitem", { name: en.roles.actions.edit }),
    ).toHaveLength(2);
    expect(
      screen.getAllByRole("menuitem", { name: en.roles.actions.delete }),
    ).toHaveLength(2);
  });
  ...
  it("app filter triggers listRolesAction with appId", async () => {
    jest.useFakeTimers();
    (actions.listRolesAction as jest.Mock).mockResolvedValue({
      items: [],
      total: 0,
      page: 1,
      pageSize: 25,
    ...
    render(withIntl(<RolesTable initial={initial} apps={apps} />));
    fireEvent.change(screen.getByLabelText(en.roles.filter.appLabel), {
      target: { value: "sq_a1" },
    });
    jest.advanceTimersByTime(400);
    await waitFor(() =>
      expect(actions.listRolesAction).toHaveBeenCalledWith({
        appId: "sq_a1",
        page: 1,
        pageSize: 25,
      }),
  ...
  it("hides Create/Edit/Delete affordances when canWrite=false", () => {
    render(
      withIntl(<RolesTable initial={initial} apps={apps} canWrite={false} />),
    );
    expect(
      screen.queryByRole("button", { name: new RegExp(en.roles.create) }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryAllByRole("menuitem", { name: en.roles.actions.edit }),
    ).toHaveLength(0);
    expect(
      screen.queryAllByRole("menuitem", { name: en.roles.actions.delete }),
    ).toHaveLength(0);
  ...
  it("hides the app picker when canPickApp=false", () => {
    render(
      withIntl(<RolesTable initial={initial} apps={apps} canPickApp={false} />),
    );
    expect(
      screen.queryByLabelText(en.roles.filter.appLabel),
    ).not.toBeInTheDocument();
  });
});
