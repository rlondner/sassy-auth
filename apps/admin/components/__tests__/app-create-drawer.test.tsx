...
describe("AppCreateDrawer", () => {
  ...
  it("submits valid form and closes", async () => {
    ...
    fireEvent.change(screen.getByLabelText(en.apps.fields.name), {
      target: { value: "X" },
    });
    fireEvent.change(screen.getByLabelText(en.apps.fields.url), {
      target: { value: "https://x.example" },
    });
    fireEvent.click(
      screen.getByRole("button", { name: en.apps.drawer.createTitle }),
    );
    await waitFor(() =>
  ...
  it("surfaces 409 nameExists error inline", async () => {
    (actions.createAppAction as jest.Mock).mockResolvedValue({
      errorKey: "apps.errors.nameExists",
    });
    ...
    await waitFor(() =>
      expect(screen.getByText(en.apps.errors.nameExists)).toBeInTheDocument(),
    );
  });
});
