    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="tabular-nums text-body-sm text-muted-foreground">
          {row.original.permissionCount} {t("roles.fields.permissionsShort")} ·{" "}
          {row.original.userCount} {t("roles.fields.usersShort")}
        </span>
      ),
    },
    {
        ...
        >
          {[5, 10, 25, 50].map((n) => (
            <option key={n} value={n}>
              {t("roles.pagination.pageSize", { count: n })}
            </option>
          ))}
        </select>
      </div>
