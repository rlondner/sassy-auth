...
| Auth server | <http://localhost:3010> |
...
```bash
curl -X POST http://localhost:3010/api/token/direct/login \
...
Verify it in your resource server against `http://localhost:3010/api/token/jwks` — see [JWKS and Token Verification](#jwks-and-token-verification), or run the [FastAPI sample](#sample-resource-server-fastapi) for a complete working consumer.
...
- `BETTER_AUTH_URL` (e.g. `http://localhost:3010`)
- `ADMIN_URL` (e.g. `http://localhost:3001`) — used to build invitation URLs sent by the API
- `AUTH_SERVER_URL` (e.g. `http://localhost:3010`) — used by the admin Server Actions
...
| `BETTER_AUTH_URL`     | Base URL of the auth server, e.g. `http://localhost:3010`. Also used as the JWT `iss` claim. |
...
| `AUTH_SERVER_URL`     | Internal URL the admin uses to reach the auth server. Default: `http://localhost:3010`      |
...
```bash
curl -X POST http://localhost:3010/api/token/oauth/token \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl http://localhost:3010/api/token/oauth/userinfo \
...
```bash
curl http://localhost:3010/api/token/jwks
```
...
const client = jwksClient({
  jwksUri: 'http://localhost:3010/api/token/jwks',
});
...
| Variable              | Description                                                   |
|-----------------------|---------------------------------------------------------------|
| `AUTH_SERVER_URL`     | SassyAuth base URL (default `http://localhost:3010`)          |
...
The admin console talks to `auth-server` via `AUTH_SERVER_URL` (default `http://localhost:3010`). All API calls forward the BetterAuth session cookie via the helpers in `apps/admin/lib/api.ts`.
